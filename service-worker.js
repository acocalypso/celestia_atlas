...
const CACHE = "celestia-atlas-offline-v35";
      ...
      await Promise.all(
        keys
          .filter(
            (key) =>
              !current.has(key) &&
              ATLAS_CACHE_PREFIXES.some((prefix) => key.startsWith(prefix)),
          )
...
function trimSurveyCache(cache) {
  surveyTrimQueue = surveyTrimQueue
    .catch(() => {})
    .then(async () => {
...
self.addEventListener("fetch", (event) => {
  ...
  if (
    url.pathname.includes("/images/dso/") ||
